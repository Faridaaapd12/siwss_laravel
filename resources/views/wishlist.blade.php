                    ...
                    <div class="row">
                        @foreach ($wishlists as $wishlist)
                            <div class="col-xl-4 col-lg-6 col-md-6">
                                <div class="box_grid">
                                    <figure>
                                        <a href="#0" onclick="unwishlist('{{ $wishlist['id'] }}')"
                                            class="wish_bt liked"></a>
                                        <a href="{{ url('package/' . $wishlist['package_id']) }}"><img
                                                src="{{ $wishlist['thumbnail'] }}" class="img-fluid" alt=""
                                                width="800" height="533"></a>
                                        <div class="read_more"><span>Read more</span></div>
                                    </figure>
                                    <div class="wrapper">
                                        <h3><a
                                                href="{{ url('package/' . $wishlist['package_id']) }}">{{ $wishlist['package_name'] }}</a>
                                        </h3>
                                        <p>{{ $wishlist['description'] }}</p>
                                        <span class="price">From <strong>{{ $wishlist['price'] }}</strong> /per
                                            person</span>
                                    </div>
                                    <ul>
                                        <li></li>
                                        <li>
                                            <div class="score"><span>Superb<em>{{ $wishlist['count'] }}
                                                        Reviews</em></span><strong>{{ $wishlist['rating'] }}</strong>
                                            </div>
                                        </li>
                                    </ul>
                                </div>
