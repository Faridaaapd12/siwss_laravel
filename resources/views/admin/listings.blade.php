        ...
        <div class="container-fluid">
                ...
                <div class="list_general">
                    <ul>
                        @foreach ($packages as $package)
                            <li>
                                <figure><img src="{{ asset($package->thumbnail) }}" alt="">
                                </figure>
                                <small></small>
                                <h4>{{ $package['package_name'] }}</h4>
                                <p>{{ $package['description'] }}</p>
                                <p><a href="{{ route('admin.listing.view', $package->id) }}" class="btn_1 gray"><i
            ...
            <nav aria-label="...">
                <ul class="pagination pagination-sm add_bottom_30">
                    ...
                    @else
                        <li class="page-item">
                            <a class="page-link" href="{{ route('admin.listings', ['page' => $currentPage - 1]) }}"
                                tabindex="-1">
                                Previous
                            </a>
                        </li>
                    ...
                    @for ($i = 1; $i <= $totalPage; $i++)
                        @if ($i == $currentPage)
                            <li class="page-item active">
                                <a class="page-link"
                                    href="{{ route('admin.listings', ['page' => $i]) }}">{{ $i }}</a>
                            </li>
                        @else
                            <li class="page-item">
                                <a class="page-link"
